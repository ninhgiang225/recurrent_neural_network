...
class GRU(Layer):
    ...
    def init_params(self, input_shape):
        ...
        # 1. Update Gate
        self.wts_update_i2h = tf.Variable(tf.random.normal([h_prev, h], 
                                          stddev=gain_gate * (h_prev**-0.5)))
        self.wts_update_h2h = tf.Variable(tf.eye(h))
        self.update_b = tf.Variable(tf.zeros([h]))

        # 2. Reset Gate (Following the same pattern as Update Gate)
        self.wts_reset_i2h = tf.Variable(tf.random.normal([h_prev, h], 
                                         stddev=gain_gate * (h_prev**-0.5)))
        self.wts_reset_h2h = tf.Variable(tf.eye(h))
        self.reset_b = tf.Variable(tf.zeros([h]))

        # 3. Candidate Gate
        self.wts_cand_i2h = tf.Variable(tf.random.normal([h_prev, h], 
                                         stddev=gain_cand * (h_prev**-0.5)))
