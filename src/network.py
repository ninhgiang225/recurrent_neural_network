...
class DeepNetwork:
    ...
    '''
    def __init__(self, input_feats_shape, loss_exp = None):
    ...
    def set_layer_training_mode(self, is_training):
        ...
        while cur_layer: 
            cur_layer.set_mode(is_training)
            cur_layer = cur_layer.prev_layer_or_block
    ...
    def init_groupnorm_params(self):
        ...
        for layer in reversed(self.layers):
            if getattr(layer, "do_group_norm", False):
                K = layer.units  # number of channels/units
        
                # Initialize gamma (scale) to 1 and beta (shift) to 0
                layer.gamma = tf.Variable(tf.ones([K]), dtype=tf.float32, trainable=True)
                layer.beta = tf.Variable(tf.zeros([K]), dtype=tf.float32, trainable=True)

                # Optionally, initialize num_groups if not set
                if not hasattr(layer, "num_groups") or layer.num_groups is None:
                    layer.num_groups = min(K // 8, 8)
    ...
    def predict(self, x, output_layer_net_act=None):
        ...
        if output_layer_net_act is None:
            output_layer_net_act = self.output_layer(x)
    ...
    def loss(self, out_net_act, y, eps=1e-16):
        ...
        B, C = tf.shape(out_net_act)[0], tf.shape(out_net_act)[1]
        if self.loss_name == 'cross_entropy':
            idx = arange_index(out_net_act, y)
            log_vals = tf.math.log(idx + eps)
            
            return -tf.reduce_mean(log_vals)
            ...
            one_hot_y = tf.one_hot(y, depth=C)
            one_hot_y = one_hot_y * 2.0 - 1.0

            # Compute |y - f(x)|^m
            abs_gap = tf.abs(one_hot_y - out_net_act)

            power_abs_gap = tf.pow(abs_gap, self.loss_exp)

            # Sum over classes, mean over batch
            return tf.reduce_mean(power_abs_gap)
        else:
            raise ValueError(f'Loss function {self.loss_name} not supported.')     
    ...
    def test_step(self, x_batch, y_batch):
        ...
        pred_classes = self.predict(x_batch, y_pred)
    ...
    def fit(self, x, y, x_val=None, y_val=None, batch_size=128, max_epochs=10000, val_every=1, print_every=10,
        ...
        recent_val_losses = []  
        recent_val_losses_for_LR_decay = []
        num_decays = 0
        ...
        for e in range(max_epochs):
            ...
            for _ in range(num_batches):
                indices = rng.choice(x.shape[0], size=batch_size, replace=True)
            ...
            training_loss = float(tf.reduce_mean(epoch_losses).numpy())
            train_loss_hist.append(training_loss)

     
            if x_val is not None and epoch_num % val_every == 0:

                val_acc, val_loss = self.evaluate(x_val, y_val)

                val_loss_float = float(val_loss.numpy())
                val_acc_float = float(val_acc.numpy())

                val_loss_hist.append(val_loss_float)
                val_acc_hist.append(val_acc_float)

                # Early stopping
                recent_val_losses, stop = self.early_stopping(
                    recent_val_losses, val_loss_float, patience
                )

                # LR decay
                recent_val_losses_for_LR_decay, do_lr_decay = self.early_stopping(
                    recent_val_losses_for_LR_decay, val_loss_float, lr_patience
                )

                if do_lr_decay and num_decays < lr_max_decays:
                    self.lr_step_decay(lr_decay_factor)
                    recent_val_losses_for_LR_decay = []
                    num_decays += 1
                    if verbose:
                        print(f"LR decayed {num_decays} times")
                ...
                if verbose and e%print_every==1:
                    print(
                        f"Epoch {epoch_num}/{max_epochs} - "
                        f"Time: {time.time() - start_time:.2f}s - "
                        f"Train Loss: {training_loss:.4f} - "
                        f"Val Loss: {val_loss_float:.4f} - "
                        f"Val Acc: {val_acc_float:.4f}"
                    )
        ...
        print(f"Learning rate decayed {num_decays} times")
        print(f"Finished training after {epoch_num} epochs!")

        return train_loss_hist, val_loss_hist, val_acc_hist, epoch_num
    ...
    def early_stopping(self, recent_val_losses, curr_val_loss, patience):
        ...
        if len(recent_val_losses) > patience + 1:
            recent_val_losses.pop(0) # Remove the oldest value
    ...
    def lr_step_decay(self, lr_decay_rate):
        ...
        print(f"Optimizer's learning rate before Step Learning rate Decay: {self.opt.learning_rate.numpy() }")
        self.opt.learning_rate *= lr_decay_rate

        print(f"Optimizer's learning after before Step Learning rate Decay: {self.opt.learning_rate.numpy() }")
