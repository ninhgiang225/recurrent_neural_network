...
def get_most_similar_words(k, word_str, all_embeddings, word_str2int, eps=1e-10):
    ...
    query_vec = all_embeddings[query_idx]       # n_units = H

    W = tf.constant(all_embeddings, dtype=tf.float32)   # M, H  all words
    w = tf.constant(query_vec, dtype=tf.float32)    # H     target words

    dot = tf.linalg.matvec(W, w) 

    W_norm = tf.norm(W, axis=1)
    w_norm = tf.norm(w)    

    cos_sim = dot / (W_norm * w_norm + eps) 

    values, indices = tf.math.top_k(cos_sim, k=k+1)
