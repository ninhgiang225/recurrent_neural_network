...
class Layer:
    ...
    def compute_net_activation(self, net_in):
        ...
        if self.activation == 'relu':
            ...
            return tf.math.tanh(self.tanh_beta * net_in)          # need check
        else:
    ...
    def __call__(self, x):
        ...
        net_in = self.compute_net_input(x)  # shape: [B, H]

        # if getattr(self, "do_group_norm", False) and hasattr(self, "gn_gain") and self.gn_gain is not None:
        if self.do_group_norm and self.gn_gain is not None:
            net_in = self.compute_group_norm(net_in)  # expects [B, H]
    ...
    def init_groupnorm_params(self):
        ...
        D = self.get_num_units()

        self.gn_gain = tf.Variable(tf.ones((D,)), trainable=True, name="gn_gain")
        self.gn_bias = tf.Variable(tf.zeros((D,)), trainable=True, name="gn_bias")
        
        self.bias = tf.Variable(0.0, trainable=False, name="bias_disabled")
...
class Dense(Layer):
    ...
    def init_params(self, input_shape):
        ...
        '''
        inputt_dim = input_shape[-1]
        if self.wt_init == 'normal':
            wts_init = tf.random.normal(shape=(inputt_dim, self.units), stddev=self.wt_scale)
        elif self.wt_init == 'he':
            k = self.get_kaiming_gain()
            he_stddev = tf.sqrt(k / tf.cast(inputt_dim, tf.float32))
            wts_init = tf.random.normal(shape=(inputt_dim, self.units), stddev=he_stddev)
        else:
            raise ValueError(f'Unsupported weight initialization method: {self.wt_init}')
        
        self.wts = tf.Variable(wts_init, trainable=True, name="wts")
    ...
    def compute_group_norm(self, net_in, eps=0.001):
        ...
        if self.num_groups is None:
            self.num_groups = min(H // 8, 8)
        
        base = H // self.num_groups
        G = self.num_groups

        net_in_reshaped = tf.reshape(net_in, [B, G, base])
        net_in_2 = (net_in_reshaped - tf.reduce_mean(net_in_reshaped, axis=-1, keepdims=True)) / (tf.math.reduce_std(net_in_reshaped, axis=-1, keepdims=True) + eps)
        # shape=(B, G H//G)
        net_in_3 = tf.reshape(net_in_2, [B, -1])
        # shape = (B, H)

        # Apply learnable gain and bias
        net_in_4 = net_in_3 * self.gn_gain + self.gn_bias
...
class Dropout(Layer):
    ...
    def compute_net_input(self, x):
        ...
        if self.get_mode():
            ...
            mask = tf.cast(tf.random.uniform(shape=tf.shape(x)) < keep_prob, dtype=tf.float32)
